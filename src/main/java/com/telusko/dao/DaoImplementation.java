...
public class DaoImplementation<T , U> implements IDaoInterface<T, U> {
    ...
    @Override
    public int create(T t, U u, LinkedHashMap<String, String> data, Model model) throws SQLException, ClassNotFoundException {
        int status ;
        if (u instanceof MysqlDatabaseOperation) {
            status = mysqlDatabaseOperation.insertIntoDatabase(t,data,model);
        }
        else {
